...
cl::opt<bool> PythonFormat("pformat", cl::value_desc("flag"), cl::desc("Use python format"), cl::init(true));
cl::opt<bool> ListOperators("ops", cl::value_desc("flag"), cl::desc("Print operators"), cl::init(false));
cl::opt<bool> ListFunctions("funs", cl::value_desc("flag"), cl::desc("Print functions"), cl::init(true));
cl::opt<bool> OnlyScalars("only-scalars", cl::value_desc("flag"), cl::desc("Print only scalars"), cl::init(true));
  ...
  for(; it != symbolTable.end(); it++) {
	...
	if ((OnlyScalars && type->isFloatingPointTy()) ||
	  ...
	  if (value->getName().find('.') == string::npos) {
	    ...
	    // To record the line number of the variable declaration
