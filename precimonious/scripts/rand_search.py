...
def is_bottom(types):
  for t in types:
    if t!="float" and t!=["float","float"]:
      return False
  return True

def random_local_search_config(change_set, type_set, switch_set, search_config, original_config, bitcode, div, original_score, count):
...
def random_search_config(change_set, type_set, switch_set, search_config, original_config, bitcode, original_score, count):
  utilities.print_config(search_config, "final_config_from_random_search.json")
  
  # search starts with the original config
  random_local_search_config(change_set, type_set, switch_set, search_config, original_config, bitcode, 2, original_score, count)
